...
class LDModel(BaseModel):
    ...
    def generate_field_context(cls) -> list[ContextTerm]:
        ...
        for field_name, field_info in cls.model_fields.items():
            field_context = next((m for m in field_info.metadata if isinstance(m, FieldContext)), None)
        ...
        for field_name, field_info in cls.model_fields.items():
            ...
            if field_context:
                ...
                if field_context.uri not in ontology.subjects():
                    raise KeyError(f"Field {field_name} has a context uri {field_context.uri} that is not in the ontology")
                
                # Check uri for field is a property
                if not any(object_type in [OWL.ObjectProperty, OWL.DatatypeProperty, OWL.AnnotationProperty, RDF.Property] for object_type in ontology.objects(field_context.uri, RDF.type )):
                    raise ValueError(f"Field {field_name} has a context uri {field_context.uri} that is not a property in the ontology")
