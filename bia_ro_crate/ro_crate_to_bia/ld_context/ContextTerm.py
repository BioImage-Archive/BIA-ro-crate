...
class ContextTerm():
    ...
    def __init__(self, full_uri: str | URIRef, field_name: str, type_mapping: Optional[str] = None):
        ...
        if prefixes:
            ...
            for prefix_name, prefix_uri in prefixes.items():
                if self.full_uri.startswith(prefix_uri):
                    ...
                    compact_id =  f"{prefix_name}:{self.full_uri.removeprefix(prefix_uri)}"
    ...
    def to_mapping_dict(self, prefixes: Optional[dict[str,str]] = None) -> dict[str, dict]:
        print(prefixes)
